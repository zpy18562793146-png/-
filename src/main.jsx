...
function ProjectPage({ project }) { return <main className="detail"><header className="detail__nav shell"><a className="brand" href="/"><b>ZHAO</b><i /> <b>PEIYI</b></a><a className="back" href="/">← 返回全部作品</a></header><section className="detail__head shell"><span>{project.number} / 08</span><p>{project.category}</p><h1>{project.title}</h1><p>本项目收录 {project.images.length} 张作品图，涵盖从概念到最终呈现的完整视觉探索。</p></section><div className="detail__gallery shell">{project.images.map((image, i) => <figure key={image} className={i === 0 ? 'detail__cover' : ''}><img src={image} alt={`${project.title} · ${i + 1}`} /><figcaption>{String(i + 1).padStart(2, '0')}</figcaption></figure>)}</div><footer className="detail__footer shell"><a href="/">← 全部作品</a><a href="#top">回到顶部 ↑</a></footer></main> }
