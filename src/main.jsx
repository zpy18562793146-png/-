...
function ProjectPage({ project }) { const details = { 'weifang-ip': ['城市文化 IP 征集项目', '将潍坊地域元素提炼为可延展的 IP 视觉语言，完成角色、应用物料与整体展示系统。', '视觉策略、IP 形象、版式与物料延展'], 'party-cultural': ['党建文创包装项目', '围绕文创产品的表达与场景化需求，建立有温度且具备识别度的包装方案。', '包装视觉、产品呈现与应用模拟'], 'aigc-awards': ['AIGC 创意图像项目', '以 AI 生成拓展创意边界，并通过人工筛选、编辑与视觉控制形成最终方案。', '创意方向、AIGC 工作流、后期整合'] }; const [context, challenge, contribution] = details[project.slug] ?? ['视觉设计项目', '从概念到最终呈现的完整视觉探索。', '视觉设计与版式呈现']; return <main className="detail"><header className="detail__nav shell"><a className="brand" href="/"><b>ZHAO</b><i /> <b>PEIYI</b></a><a className="back" href="/">← 返回全部作品</a></header><section className="detail__head shell"><span>{project.number} / 08</span><p>{project.category}</p><h1>{project.title}</h1><div className="case-summary"><div><span>项目背景</span><p>{context}</p></div><div><span>设计策略</span><p>{challenge}</p></div><div><span>我的贡献</span><p>{contribution}</p></div><div><span>作品收录</span><p>{project.images.length} 张视觉成果图</p></div></div></section><div className="detail__gallery shell">{project.images.map((image, i) => <figure key={image} className={i === 0 ? 'detail__cover' : ''}><img src={image} alt={`${project.title} · ${i + 1}`} /><figcaption>{String(i + 1).padStart(2, '0')}</figcaption></figure>)}</div><footer className="detail__footer shell"><a href="/">← 全部作品</a><a href="#top">回到顶部 ↑</a></footer></main> }
